...
import javax.swing.*;
public class employee1 extends JFrame implements ActionListener,KeyListener
{
	Container c;
	JPanel P1,P2;
...
;
	...
	//save button b2
		...
		q="insert into tbl_employee values("+Integer.parseInt(id)+",'" + name +"','"+addr+"','"+city+"','"+pincode+"','"+email+"','"+mno+"')";
		st.executeUpdate(q);
		con.close();
		...
		if(ae.getSource()==b1)
		{
			...
			st=con.createStatement();
			q="select max(empid) from tbl_employee";
