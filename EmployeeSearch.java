...
import javax.swing.*;
public class EmployeeSearch extends JFrame implements ActionListener,KeyListener,ItemListener
{
	Container c;
	JPanel P1,P2;
		...
		public void itemStateChanged(ItemEvent ie) {
			...
			String strid=(String)ie.getItem();
			   try
				...
				con=DriverManager.getConnection("jdbc:mysql://localhost:3306/green_Bicycle_DB","root","root");
				st=con.createStatement();
				q="select * from tbl_employee where empid="+ Integer.parseInt(strid)+"";
				rs=st.executeQuery(q);
				while(rs.next())
				{
				...
				con.close();
						...
						}catch(Exception e) { System.out.println("Connection driver loaded"+e);}
	...
	//update button b2
		...
		con=DriverManager.getConnection("jdbc:mysql://localhost:3306/green_Bicycle_DB","root","root");
		st=con.createStatement();
		q="update tbl_employee set empname='" + name +"',empadd='"+addr+"',city='"+city+"',pincode='"+pincode+"',email='"+email+"',mobile='"+mno+"' where empid="+Integer.parseInt(id)+"";
		st.executeUpdate(q);
		con.close();
		JOptionPane.showMessageDialog(null,"Values are updated","Update", JOptionPane.INFORMATION_MESSAGE);
...
{
EmployeeSearch ob=new EmployeeSearch();
}
