...
public class customer extends JFrame  implements ActionListener,KeyListener
{
	...
	JButton b1,b2,b3,b4,b5,b6,bshow;
	...
	//save button b2
		if(ae.getSource()==b2)
		{
						...
						name=t2.getText();
						addr=t3.getText();
						city=(String)t4.getSelectedItem();
						pincode=t5.getText();
						email=t6.getText();
						mno=t7.getText();
						int l=email.indexOf("@");
		...
		q="insert into tbl_customer values("+Integer.parseInt(id)+",'" + name +"','"+addr+"','"+city+"','"+pincode+"','"+email+"','"+mno+"')";
		st.executeUpdate(q);
		con.close();
		...
		if(ae.getSource()==b1)
		{
			...
			st=con.createStatement();
			q="select max(cid) from tbl_customer";
